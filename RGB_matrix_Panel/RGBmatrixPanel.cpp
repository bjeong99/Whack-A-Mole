...
void RGBmatrixPanel::updateDisplay(void)
{
    ...
    if(++plane >= nPlanes) {        // Advance plane counter.  Maxed out?
        ...
        if(++row >= nRows) {        // advance row .  Maxed out?
        ...
        //_rowBus.write(row);
        pinWrite(PTB, 2, row&0x01); //a
        pinWrite(PTB, 3, row>>1&0x01); //b
        pinWrite(PTB, 10, row>>2&0x01);//c
        pinWrite(PTB, 11, row>>3&0x01);//d
    }
    //_oe=0;
    pinWrite(PTC, 17, 0);
    //_latch=0;
    ...
    if(plane > 0) {
        for(int i=0; i<32; i++) {
            //_dataBus.write((*(buffptr+i)>>2));
                        //_sclk=1;
            pinWrite(PTC, 2, 1);
            pinWrite(PTC, 11, (*(buffptr+i)>>2)&0x01);//r1
            pinWrite(PTC, 10, (*(buffptr+i)>>2)>>1&0x01);//g1
            pinWrite(PTB, 9, (*(buffptr+i)>>2)>>2&0x01);//b1
            pinWrite(PTA, 1, (*(buffptr+i)>>2)>>3&0x01);//r2
            pinWrite(PTB, 23, (*(buffptr+i)>>2)>>4&0x01);//g2
            pinWrite(PTA, 2, (*(buffptr+i)>>2)>>5&0x01);//b2
            //_sclk=0;
            for(int i=0; i<10; i++) {}
            pinWrite(PTC, 2, 0);
        }
        ...
        for(int i=0; i<32; i++) {
            //_dataBus.write(((buffptr[i]<<4)|((buffptr[i+32]<<2)&0x0C)|((buffptr[i+64])&0x03)));
                        //_sclk=1;
            pinWrite(PTC, 2, 1);
            pinWrite(PTC, 11,((buffptr[i]<<4)|((buffptr[i+32]<<2)&0x0C)|((buffptr[i+64])&0x03))&0x01);//r1
            pinWrite(PTC, 10,((buffptr[i]<<4)|((buffptr[i+32]<<2)&0x0C)|((buffptr[i+64])&0x03))>>1&0x01);//g1
            pinWrite(PTB, 9, ((buffptr[i]<<4)|((buffptr[i+32]<<2)&0x0C)|((buffptr[i+64])&0x03))>>2&0x01);//b1
            pinWrite(PTA, 1, ((buffptr[i]<<4)|((buffptr[i+32]<<2)&0x0C)|((buffptr[i+64])&0x03))>>3&0x01);//r2
            pinWrite(PTB, 23, ((buffptr[i]<<4)|((buffptr[i+32]<<2)&0x0C)|((buffptr[i+64])&0x03))>>4&0x01);//g2
            pinWrite(PTA, 2, ((buffptr[i]<<4)|((buffptr[i+32]<<2)&0x0C)|((buffptr[i+64])&0x03))>>5&0x01);//b2
            //_sclk=0;
            pinWrite(PTC, 2, 0);
            for(int i=0; i<10; i++) {}
        }
